//! Full-sequence equivalence of the N-phase machine over the REAL fixture
//! proof: begin → claim chunks → claim finalize → lookup chunks → lookup
//! finalize → OODS+mix → FRI commit → fused Merkle/fri_answers group txs →
//! FRI decommit + fact — with a checkpoint serde round-trip between every
//! pair of transactions — must produce exactly the monolithic verifier's
//! output, hitting the same channel digests at the phase seams.
...
/// Fused Merkle+fri_answers group files (bridge split-witness, group 16).
...
/// the head (claim with a 6-entry program prefix + small sections + queries
/// PoW nonce + salt) — exactly what the client-side serializer will emit.
fn build_streams(values: Span<felt252>) -> Streams {
    ...
    // Head: claim with the program truncated to HEAD_PROGRAM_ENTRIES,
    // then [interaction_pow, icl, config, commitments], pow nonce, salt.
...
/// Loads one bridge witness-group fixture (same layout as
/// test_witness_groups).
...
#[test]
fn test_machine_full_sequence_matches_monolithic() {
...
fn test_machine_rejects_tampered_head() {
    let file = FileTrait::new("tests/data/poseidon_chain_n100_full_proof_packed_v2.txt");
    let slots = read_txt(@file);
    let values = unpack_proof_v2(slots.span(), N_VALUES);
    let streams = build_streams(values.span());
    ...
    // Any head byte change must be rejected by the digest saved at begin.
...
#[test]
#[should_panic(expected: 'lookup chunk binding')]
fn test_machine_rejects_lookup_boundary_mismatch() {
    let file = FileTrait::new("tests/data/poseidon_chain_n100_full_proof_packed_v2.txt");
    let slots = read_txt(@file);
    let values = unpack_proof_v2(slots.span(), N_VALUES);
    let streams = build_streams(values.span());
    ...
    // Claim phase feeds one big chunk…
    let claim_state = machine_begin(streams.head.span(), n_entries);
    let claim_state = machine_claim_chunk(claim_state, program);
    let lookup_state = machine_claim_finalize(claim_state, streams.head.span());

    // …but the lookup phase re-supplies at different boundaries: same
    // bytes, different roll — must be rejected (chunk boundaries are part
    // of the binding).
    let half = n_entries / 2;
    let lookup_state = machine_lookup_chunk(lookup_state, program.slice(0, half));
    let lookup_state = machine_lookup_chunk(
        lookup_state, program.slice(half, n_entries - half),
    );
    let _ = machine_lookup_finalize(lookup_state, streams.head.span());
}
